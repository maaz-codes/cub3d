...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/03/04 18:00:45 by rcreer            #+#    #+#             */
/*   Updated: 2025/03/04 18:01:59 by rcreer           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	apppend_file(t_parsing *parse, int *row, char *file_check, int i)
{
	...
	{
		parse->file_data[*row] = ft_calloc(ft_strlen(file_check) + 1,
				sizeof(char *));
		if (!parse->file_data[*row])
			return (0);
		ft_strlcpy(parse->file_data[*row], file_check,
			ft_strlen(file_check) + 1);
		(*row) += 1;
		return (1);
	}
	return (0);
